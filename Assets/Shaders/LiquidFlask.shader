    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings
            {
                ...
                float4 positionWS  : TEXCOORD0; // xyz = worldPos, w = localY
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // Идентификация геометрической крышки меша
                bool isMeshLid = IN.positionWS.w > (_LocalMeshMax - 0.001);
                
                float3 finalColor;
                if (facing < 0) {
                    // Внутренняя часть (задние грани). 
                    // Если это внутренняя сторона крышки меша — отбрасываем, чтобы не было наложения (Z-fight)
                    if (isMeshLid) discard;
                    
                    // Рисуем плоскость поверхности
                    finalColor = _SurfaceColor.rgb;
                } else {
                    // Внешняя часть (передние грани)
                    float foam = 1.0 - smoothstep(0.0, _FoamWidth, clipVal);
                    finalColor = lerp(liquidCol, _SurfaceColor.rgb, foam);
                    
                    // Если это сама крышка меша, красим её в цвет поверхности
                ...
                return half4(MixFog(finalColor, IN.fogFactor), 1.0);
            }
ENDHLSL
        }
