namespace HierarchyGadget { // v1.2.0
	...
	public class HierarchyGadget {
		...
		public static void HieGUI (int instanceID, Rect rect) {
			...
			//float globalOffsetX = PrefabUtility.IsAnyPrefabInstanceRoot(obj) == PrefabInstanceStatus.NotAPrefab ? 0 : -16;
			//float globalOffsetX = PrefabUtility.IsAnyPrefabInstanceRoot(obj) == false ? 0 : -16;
			float globalOffsetX = -14;
			...
			if (ShowActiveToggle) {
				// Active Toggle
				rect.x = rect.width + ( globalOffsetX + iconSize + 2);
