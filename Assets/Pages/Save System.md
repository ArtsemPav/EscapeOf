...
Система использует `DefaultExecutionOrder` чтобы гарантировать правильный порядок загрузки при старте сцены:
...
| `MedallionCollectionTracker` | `-5` | Синхронизирует порядок подбора медальонов с инвентарём |
| Остальные ISaveable | `0` | Стандартный `Start()` |

**Почему порядок критичен:** если `ISaveable` вызывает `SaveManager.Save()` в своём `Start()` раньше, чем другие объекты применили `LoadSaveData()` — снимок состояния будет неполным. Он зафиксирует чужие данные как пустые и перезапишет корректный файл.

**Правило:** любой `ISaveable`, чей `Start()` может вызвать `Save()`, должен иметь порядок **строго больший** чем у всех объектов, чьи данные он может захватить в снимок.

**Паттерн `_isReady`:** чтобы синхронизация при старте не инициировала преждевременное сохранение:
...
private void Start()
{
    // Синхронизация без сохранения — другие ISaveable ещё не применили LoadSaveData()
    SyncState();
    _isReady = true;
}
...
**Дублирование данных после перезагрузки (объект оказывается в двух местах одновременно)**
- Race condition при инициализации: один `ISaveable` сохраняет снимок до того, как другой применил `LoadSaveData()`. Смотри раздел **Порядок инициализации и ISaveable** выше.
- Убедись что проблемный компонент использует паттерн `_isReady` и что его `DefaultExecutionOrder` правильно выставлен.
